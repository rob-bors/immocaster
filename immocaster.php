...
Version: 1.3.0
Author: Norman Braun
Author URI: http://www.pixelairport.de/
Min WP Version: 3.0
*/

// Configuration
define('IMMOCASTER_PLUGIN_VERSION','1.3.0');
...
function immocaster_redirect()
{
	...
	{
		$sLink = base64_decode($_GET[IMMOCASTER_GET_PARAM_REDIRECT]);
...
// Dashboard widget
function immocaster_dashboard_widget_function()
{
	include_once(ABSPATH . WPINC . '/class-IXR.php');
	$sBoxText = __('You will find all news about immocaster on <a href="http://www.immocaster.com" target="_blank">www.immocaster.com</a>.');
	$ixrcli = new IXR_Client('http://www.immocaster.com/xmlrpc.php');
	if($ixrcli->query('plugin.info.wordpress',IMMOCASTER_PLUGIN_VERSION,get_bloginfo('url'),get_bloginfo('version'),IMMOCASTER_PRO_LICENCE,get_bloginfo('language')))
	{
		$sBoxText = $ixrcli->getResponse();
	}
	echo '<table cellpadding="0" cellspacing="0" border="0"><tr><td><img src="'.
	plugins_url('images/immocaster_icon_med.png',__FILE__).'"></td><td>'.$sBoxText.'</td></tr></table>';
} 
function immocaster_add_dashboard_widgets()
{
	wp_add_dashboard_widget('immocaster_dashboard_widget',__('Immocaster'),'immocaster_dashboard_widget_function');	
} 
add_action('wp_dashboard_setup', 'immocaster_add_dashboard_widgets' );
